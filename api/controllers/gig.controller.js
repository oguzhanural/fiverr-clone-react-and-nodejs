...
export const getGig = async (req,res,next) => {
    try {
        ...
        if(!getGig) return next(createError(404, "Gig  not found!"));
        res.status(200).send(getGig);
    } catch (error) {
        next(error);
    }
...
export const getGigs = async (req,res,next) => {
    ...
    const filters = {
        ...
        ...(q.search && {title: { $regex: q.search, $options: "i" } } ) // $option: "i" büyük ya da küçük harf duyarlılığını görmezden gelir.
    };
    try {
